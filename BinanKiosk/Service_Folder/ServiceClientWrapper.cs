...
namespace BinanKiosk.Service_Folder
{
			...
			try
			{
				...
				using (var client = new HttpClient())
				{
					var response = request.HttpProtocol.Equals(Protocols.HTTP_GET) ? client.GetAsync(baseAddress).Result :
						request.HttpProtocol.Equals(Protocols.HTTP_POST) ? client.PostAsync(baseAddress, new StringContent(request.Body ?? string.Empty, Encoding.UTF8, "application/json")).Result :
						client.PutAsync(baseAddress, new StringContent(request.Body ?? string.Empty, Encoding.UTF8, "application/json")).Result;
